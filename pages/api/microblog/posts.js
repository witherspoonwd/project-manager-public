import mysql from "mysql2/promise"

export default async function handler(req, res) {

  const dbconnection = await mysql.createConnection({
    host: process.env.SQL_HOST,
    database: process.env.SQL_DB,
    user: process.env.SQL_USR,
    password: process.env.SQL_PASS,
    socketPath: process.env.SQL_SOCK
  });

  try {
    const pageNum = parseInt(req.headers['page'], 10);
    //const page = Number(req.query.page);

    let data;

    if (!pageNum) {
      const query = "SELECT postDateTime, postTitle, postID FROM microblog_posts ORDER BY postID DESC";
      const values = [];

      data = await dbconnection.execute(query, values);
      dbconnection.end();
    }

    else {
      const indexFrom = String((pageNum - 1) * 25);
      const amountToDisplay = "25";

      const query = "SELECT postTitle, postDateTime, postID FROM microblog_posts WHERE isViewable=1 ORDER BY postID DESC LIMIT ?, ?";
      const values = [indexFrom, amountToDisplay];

      data = await dbconnection.execute(query, values);
      dbconnection.end();
    }

    // clean up
    const cleanData = data[0].map((item) => ({
      date: item.postDateTime,
      title: item.postTitle,
      postID: item.postID
    }));

    res.status(200).json(cleanData);
  }

  catch(error){
    res.status(500).json(error);
  }
}