...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!pageNum) {
      const query = "SELECT noteID, noteTitle, dateCreated, isArchived FROM private_notepad ORDER BY dateCreated DESC";
      const values = [];
    ...
    // one day we will implement page numbers on the songs page. i hope to write thousands of songs. all to be in this database one day.
      ...
      const query = "SELECT noteID, noteTitle, dateCreated, isArchived FROM private_notepad WHERE isArchived IS NULL ORDER BY dateUpdated DESC LIMIT ?, ?";
